...
- `prefill-kv-16-prefill-protected` compiles for watchOS and restores Swift decode top-1 when paired with either fp16 or int8 decode on `en-short-001`. In the 5-prompt capped Swift CLI batch it keeps average token agreement at 0.80, fixes `en-short-001` and `watch-utility-001`, but regresses `code-fix-001`; it is a quality-direction candidate, not yet a default policy.
- `prefill-kv-16-prefill-protected-no-int4` compiles for watchOS and reaches 1.0 average token agreement on the same 5-prompt capped Swift CLI batch when paired with `decode-16-int8`. It is 1,252,480,141 bytes, only about 10.6MB larger than the layer-12-int4 protected variant, and is the current best context-16 fidelity baseline.
